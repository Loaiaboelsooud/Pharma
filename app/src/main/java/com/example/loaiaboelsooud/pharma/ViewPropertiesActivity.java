...
public class ViewPropertiesActivity extends NavMenuInt implements HTTPRequests.GetPropertiesList, PropertiesAdapter.OnPropertiesClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (isFiltered) {
            ...
            httpRequests.sendPropertiesFilterGetRequest(prefUtil.getToken(), SELLING, RENTING, PHARMACY, WAREHOUSE, HOSPITAL, FACTORY, this, 1);
        } else {
    ...
    private void loadMore(MetaData metaData) {
        if (metaData.getPagination().getTotalPages() > metaData.getPagination().getCurrentPage()) {
            progressBar.setVisibility(View.VISIBLE);
            if (isFiltered) {
                httpRequests.sendPropertiesFilterGetRequest(prefUtil.getToken(), SELLING, RENTING, PHARMACY, WAREHOUSE, HOSPITAL, FACTORY,
